"""Incrementally collect Westmetall LME zinc data.

The first run downloads every available year from 2008 onward. Later runs start
at the year containing the newest locally stored observation, preserving older
history while refreshing source revisions and newly published rows.
"""
...
class TableRowParser(HTMLParser):
    """Extract text cells from HTML table rows using the standard library."""
...
def download(url: str, timeout: int, retries: int) -> bytes:
    request = Request(
        url,
        headers={
            "User-Agent": "ZincResearchCollector/1.0 (yearly archive requests)",
...
def parse_page(html: bytes, year: int, fetched_at: str, url: str) -> list[dict[str, str]]:
    ...
    if not records:
        raise ValueError(f"No zinc data rows found on the page for {year}; layout may have changed")
...
def read_existing(csv_path: Path) -> dict[str, dict[str, str]]:
    ...
    if len(rows) != len({row["date"] for row in rows}):
        raise ValueError(f"Duplicate dates in existing dataset: {csv_path}")
    if [row["date"] for row in rows] != sorted(row["date"] for row in rows):
        raise ValueError(f"Existing dataset is not sorted by date: {csv_path}")
    return {row["date"]: row for row in rows}


def write_csv_atomic(csv_path: Path, records: dict[str, dict[str, str]]) -> None:
...
def collect(project_dir: Path, timeout: int, retries: int, delay: float) -> Path:
    raw_dir = project_dir / "data" / "raw" / "lme"
    snapshot_dir = raw_dir / "html_snapshots"
    csv_path = raw_dir / "zinc_lme_raw.csv"
    existing = read_existing(csv_path)
    current_year = date.today().year
    newest_date = max(existing) if existing else None
    start_year = max(
        int(newest_date[:4]) if newest_date else FIRST_AVAILABLE_YEAR,
    ...
    for index, year in enumerate(years):
        ...
        snapshot_path = snapshot_dir / f"zinc_{year}_{timestamp}.html"
