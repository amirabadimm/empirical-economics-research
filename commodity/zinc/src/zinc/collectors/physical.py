"""Collect every zinc-related physical-market row from the official IME API.

The broad raw scope keeps every row whose normalized GoodsName contains «روی»,
including zinc ingot grades, zinc soil, all producers, symbols, contracts,
settlements, and zero-quantity offers. Underlying selection is a later step.
"""
...
def is_zinc_related(row: dict) -> bool:
    return "روی" in normalize_fa(row.get("GoodsName"))
