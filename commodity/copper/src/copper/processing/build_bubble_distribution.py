"""Build standardized empirical distributions for every copper bubble series."""
...
SPECS = (
    BubbleSeriesSpec("certificate_physical", "certificate_vs_physical", "Certificate vs physical", "copper_certificate_bubble.csv", "date", "certificate_bubble_pct", point_method_column="physical_ratio_method"),
...
def build():
    return build_project_distributions(project_dir=PROJECT_DIR, commodity="copper", specs=SPECS, half_life_days=HALF_LIFE_DAYS)


if __name__ == "__main__":
    result = build()
    print(f"Built {len(result)} copper distribution rows")
