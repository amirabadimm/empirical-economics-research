"""Build one simple regression output from LME*USD intrinsic copper value."""
...
def main() -> None:
    ...
    if len(anchors) < 6:
        raise ValueError(f"At least six exact anchors are required for five time-series splits; found {len(anchors)}")
    ...
    for name, model in models.items():
        ...
        for train_index, test_index in splitter.split(x):
            fold_model = clone(model).fit(x.iloc[train_index], y.iloc[train_index])
    ...
    output["certificate_bubble_pct"] = (
        output["certificate_price_irr_per_kg"]
        / output["estimated_physical_price_irr_per_kg"]
        - 1
    ) * 100
