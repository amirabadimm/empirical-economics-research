"""Shared time-series model selection for certificate/physical intrinsic-value analysis."""
...
def build(project: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    anchors = (
        ...
        .merge(
            physical[["date", "physical_price_irr_per_kg"]],
            on="date",
            how="inner",
            validate="one_to_one",
        )
    ...
    for name, model in models.items():
        ...
        for train_index, test_index in splitter.split(x):
            prediction.iloc[test_index] = (
                clone(model)
                .fit(x.iloc[train_index], y.iloc[train_index])
                .predict(x.iloc[test_index])
            )
    ...
    output = (
        certificate[["date", "intrinsic_price_irr_per_kg", "certificate_price_irr_per_kg"]]
        .copy()
        .merge(anchors, on="date", how="left", validate="one_to_one")
    )
    ...
    output = output[
        [
            ...
            "is_actual_physical_observation",
            "selected_model",
        ]
    ]
    metrics = pd.DataFrame(
        [
            {"model": name, "timeseries_cv_rmse": value, "selected": int(name == selected_name)}
    ...
    metrics.to_csv(
        processed / "intrinsic_regression_metrics.csv", index=False, encoding="utf-8-sig"
    )
