"""Incremental collector for the IME PistaCL certificate."""
...
MARKET_ID = 22
FIRST_QUERY_DATE = date(2026, 8, 25)
MAX_CHUNK_DAYS = 180
PAGE_SIZE = 100
RAW_COLUMNS = [
...
class CertificateConfig:
    ...
    new_code: str
    csv_name: str
    first_query_date: date = FIRST_QUERY_DATE
...
def collect(project_dir: Path, config: CertificateConfig, timeout: int, retries: int,
    ...
    if existing and not full_refresh:
        start = max(config.first_query_date, date.fromisoformat(max(existing)) - timedelta(days=refresh_days))
    else:
        start = config.first_query_date
    if start > today:
    ...
    for chunk_start, chunk_end in chunks(start, today):
        ...
        snapshot = {
            ...
            "commodity": {**config.__dict__, "first_query_date": config.first_query_date.isoformat()},
...
CONFIG = CertificateConfig(
    slug="pista",
    title_fa="گواهی سپرده پیوسته پسته",
    commodity_id="11",
    contract_description="گواهی سپرده پیوسته پسته",
    old_code="PistaCL",
    new_code="PistaCL",
    csv_name="pista_certificate_raw.csv",
)
